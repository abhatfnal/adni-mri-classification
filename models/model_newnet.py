...
class BasicBlock1(nn.Module):
    ...
    def __init__(self, channel_in):
        ...
        self.residual1 = nn.Sequential(
            nn.Conv3d(channel_in, channel_in, 3, padding=3//2, bias=False),
            StdNorm3d(),
            nn.ReLU(),
            nn.Conv3d(channel_in, channel_in, 3, padding=3//2, bias=False),
            StdNorm3d(),
...
class BasicBlock2(nn.Module):
    ...
    def __init__(self, channel_in):
        ...
        self.residual1 = nn.Sequential(
            nn.Conv3d(channel_in, channel_out, 3, stride=2, padding=3//2, bias=False),
            StdNorm3d(),
            nn.ReLU(),
            nn.Conv3d(channel_out, channel_out, 3, padding=3//2, bias=False),
            StdNorm3d(),
            SAM3d()
        )
        
        # First skip connection (linear map)
        self.skip1 = nn.Conv3d(channel_in, channel_out, 1, stride=2, padding=0, bias=False)
