...
class MyNet2(BaseModel):
    def _build(self):
        ...
        # Residual block -> Channel droput -> MaxPool
        self.resblock = ResidualBlock3D(16, 32)
        self.dropout1 = nn.Dropout3d(0.3)
        self.pool2 = nn.MaxPool3d(2, 2)

        # Bottleneck conv -> (CBAM final) -> GAP -> FC
        self.conv4 = nn.Conv3d(32, bottleneck_channels, kernel_size=3, padding=1)
        self.bn4   = nn.BatchNorm3d(bottleneck_channels)
    ...
    def forward(self, x):
        ...
        self.gap_out = self.gap(x).view(x.size(0), -1)
